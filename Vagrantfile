...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell" do |s|
    ...
    s.inline = localscriptDir + "/create-postgres.sh " + dbName + " " + dbUser + " " + dbPassword
  end
  ...
  config.vm.provision "shell" do |s|
    s.name = "Installing pgBouncerHud REST api framework"
    s.inline = localscriptDir + "/install_api_sources.sh"
  end

  config.vm.provision "shell" do |s|
    s.name = "Setup pgBouncerHud REST framework"
  ...
  config.vm.provision "shell" do |s|
      ...
      s.inline = localscriptDir + "/serve-nginx.sh" + " pgbouncerhud.app"+ " /var/www/pgbh-api/public" + " 80"+ " 443"
  end
